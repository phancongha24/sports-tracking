...
Do not reuse the same `gameId` inside one coupon unless the user explicitly wants correlation testing. Verify with `/api/xbet/coupon?verify=1&summ=100000&cookieMode=auto`; if a leg is removed, replace it and verify again.
...
Keep output concise. Include snapshot, window, scanned count, table of candidates, coupon verification if applicable, and a sure-win claim audit when requested. Use labels `unsupported`, `weak`, `plausible`, and `strong-but-not-certain`; never output a binary guarantee. Add a short note that this is data analysis only, not a guarantee or betting instruction.
